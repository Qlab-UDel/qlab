...
ggplot(subset2,aes(y=rt_slope,x=Language))+geom_point(color='sienna') + geom_smooth(method='lm',color="sienna1") + labs(x="WJ4 Score", y="Response time (ms)",title="RT slope by Language score in vsl") + ylim(c(-15,15))
...
for(id in scf$ID){ vsl_acc_table$gender[which(vsl_acc_table$acc_id==id)]<-paste(scf$Gender[scf$ID==id])}
t.test(vsl_acc_table$subj_corr[vsl_acc_table$gender=="Female"],vsl_acc_table$subj_corr[vsl_acc_table$gender=="Male"])
