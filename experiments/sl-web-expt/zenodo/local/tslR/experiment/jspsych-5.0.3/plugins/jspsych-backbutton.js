 /* jspsych-backbutton.js
 * An Nguyen
 *
 * This plugin allows user to go back to the previous trial
 * Use it to show backbutton, provide performance feedback, etc...
 *
 * 
 *
 */

jsPsych.plugins.backbutton = (function() {
  ...
  plugin.trial = function(display_element, trial) {
	...
	current_stim = trial.pages[(current_page-1)];
      ...
      if (trial.show_clickable_nav) {
        ...
        var nav_html = "<div class='jspsych-backbutton-nav'>";
        if (current_page != 0 && trial.allow_backward) {
          nav_html += "<button id='jspsych-backbutton-back' class='jspsych-btn'>&lt; Previous</button>";
        }
        nav_html += "<button id='jspsych-backbutton-next' class='jspsych-btn'>Next &gt;</button></div>"

        display_element.append(nav_html);

        if (current_page != 0 && trial.allow_backward) {
          $('#jspsych-backbutton-back').on('click', function() {
            clear_button_handlers();
            back();
          });
        }

        $('#jspsych-backbutton-next').on('click', function() {
          clear_button_handlers();
          next();
        });
    ...
    function clear_button_handlers() {
      $('#jspsych-backbutton-next').off('click');
      $('#jspsych-backbutton-back').off('click');
    }
