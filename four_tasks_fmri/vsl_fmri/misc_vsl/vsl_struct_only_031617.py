...
expName = u'vsl_struct_only_031617'  # from the Builder filename that created this script
expInfo = {u'PartID': u'', u'target': u'', u'language': u''}
...
filename = _thisDir + os.sep + u'data/%s_%s_%s' %(expInfo['PartID'], expName, expInfo['date'])
...
fam_block_trials = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions(u'fam_seq_%s.xlsx' % str(expInfo['language'])),
...
forced_test_1_block = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions(u'vsl_forced_test_%s.xlsx' % str(expInfo['language'])),
