...
mycor_early <- rcorr(as.matrix(df_corr),type = "spearman")
...
```{r, echo=TRUE}
#df_corr<- na.omit(df_corr)
mycor_late <- rcorr(as.matrix(df_corr),type = "spearman")
...
### Plotting the early correlation
```{r}
ggplot(df_corr, aes(x=High_Low_MMN, y=accuracy)) + geom_point(size=3, color="navy blue", shape=19) + labs(title="Distributional SL Accuracy & Global (p) MMN", x="Global (p) MMN", y = "SL Accuracy") + theme(plot.title = element_text(hjust = 0.5)) + geom_smooth(method=lm)
```

### Plotting the late correlation
```{r}
ggplot(df_corr, aes(x=Short, y=scaled_rt_slope)) + geom_point(size=3, color="navy blue", shape=19) + labs(title="Distributional SL RT & Local (p)", x="Local (p) Short Only", y = "RT Slope") + theme(plot.title = element_text(hjust = 0.5)) + geom_smooth(method=lm)
```

### Plotting the late correlation
```{r}
ggplot(df_corr, aes(x=Short_Long_MMN, y=scaled_rt_slope)) + geom_point(size=3, color="navy blue", shape=19) + labs(title="Distributional SL RT & Local (p) MMN", x="Local (p) MMN", y = "RT Slope") + theme(plot.title = element_text(hjust = 0.5)) + geom_smooth(method=lm)
```