...
```{r, echo=TRUE}
model<- lmer(Late~ 1 + Domain*Global*Local + (1+(Global*Local)|subject),data = df_factor,control = lmerControl(calc.derivs = FALSE))
summary(model)
```
...
```{r, echo=TRUE}
late_blastANOVA <- ezANOVA(df_factor, Late, subject, within = .(Domain,Local,Global), type = 3, detailed = TRUE)
late_blastANOVA
```
...
pairwise.t.test(blast_short$Late, blast_short$Global, p.adj = 'bonferroni')
pairwise.t.test(blast_long$Late, blast_long$Global, p.adj = 'bonferroni')

pairwise.t.test(blast_high$Late, blast_high$Local, p.adj = 'bonferroni')
...
df_corr$High_Low_MMN = df_corr$Low - df_corr$High
...
df_corr$Short_Long_MMN = df_corr$Long-df_corr$Short
...
```{r}
cor.test(df_corr$accuracy, df_corr$scaled_rt_slope,  method = "pearson", use = "complete.obs")
# Be sure to run the corr matrix for the early time window
cor.test(df_corr$Short_Long_MMN, df_corr$scaled_rt_slope,  method = "spearman", use = "complete.obs")

# Be sure to run the corr matrix for the late time window
cor.test(df_corr$Global_long, df_corr$scaled_rt_slope,  method = "spearman")
cor.test(df_corr$Global_short, df_corr$scaled_rt_slope,  method = "spearman")
cor.test(df_corr$Local_high, df_corr$scaled_rt_slope,  method = "spearman")
cor.test(df_corr$Local_low, df_corr$scaled_rt_slope,  method = "spearman")
```
...
```{r}
ggplot(df_corr_early, aes(x=Local_low, y=scaled_rt_slope)) + geom_point(size=3, color="navy blue", shape=19) + labs(title="Relationship between MMN and Learning", x="Early ERP Amplitude", y = "Speech SL (RT Slope)") +   theme_classic() +
...
```{r}
ggplot(df_corr_late, aes(x=Local_high, y=scaled_rt_slope)) + geom_point(size=3, color="navy blue", shape=19) + labs(title="RT Slope & Local (p) within High Condition", x="Local (p) MMN within High frequency Condition", y = "RT SLope") +   theme_classic() +
theme(plot.title = element_text(hjust = 0.5))
```
