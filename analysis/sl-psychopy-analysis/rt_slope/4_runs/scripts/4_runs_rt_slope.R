...
old_auditory_files <- gsub("_", "", old_afiles)
new_auditory_files <- gsub("_", "", new_afiles)
...
old_auditory_data_frame<-list()
new_auditory_data_frame<-list()

# Remove the dashes in each file name read it in
for(old_auditory_file in old_afiles)
{
  assign(
    gsub("_", "", old_auditory_file),
    read.csv(paste(old_auditory_path, old_auditory_file, sep="")))
}

# Combine each file with the previous files into auditory_data_frame
for (old_afile in old_auditory_files){old_auditory_data_frame <- append(old_auditory_data_frame, list(eval(parse(text=old_afile))))}
old_auditory_data_frame <- do.call(rbind.data.frame, old_auditory_data_frame)

# Remove the dashes in each file name read it in
for(new_auditory_file in new_afiles)
{
  assign(
    gsub("_", "", new_auditory_file),
    read.csv(paste(new_auditory_path, new_auditory_file, sep="")))
}

# Combine each file with the previous files into auditory_data_frame
for (new_afile in new_auditory_files){new_auditory_data_frame <- append(new_auditory_data_frame, list(eval(parse(text=new_afile))))}
new_auditory_data_frame <- do.call(rbind.data.frame, new_auditory_data_frame)
...
old_auditory_data_frame$soundFile <- gsub (".wav", "", old_auditory_data_frame$soundFile, ignore.case=TRUE)

# Convert targets and soundFile from factors to atomic variables 
old_auditory_data_frame$ttarget<-as.character(old_auditory_data_frame$ttarget)
old_auditory_data_frame$soundFile<-as.character(old_auditory_data_frame$soundFile)
old_auditory_data_frame$starget<-as.character(old_auditory_data_frame$starget)

# Rename expName column as "modality"
names(old_auditory_data_frame)[names(old_auditory_data_frame) == 'expName'] <- 'modality'

# Match name of image with name of target in auditory files by removing extension from end of sound_file
new_auditory_data_frame$soundFile <- gsub (".wav", "", new_auditory_data_frame$soundFile, ignore.case=TRUE)

# Convert targets and soundFile from factors to atomic variables 
new_auditory_data_frame$ttarget<-as.character(new_auditory_data_frame$ttarget)
new_auditory_data_frame$soundFile<-as.character(new_auditory_data_frame$soundFile)
new_auditory_data_frame$starget<-as.character(new_auditory_data_frame$starget)

# Rename expName column as "modality"
names(new_auditory_data_frame)[names(new_auditory_data_frame) == 'expName'] <- 'modality'
...
# match name of image with name of target in visual files  ----------------------------------------------------------------------------------------
...
# Return ltarget of "F" to original name
visual_data_frame$ltarget <- gsub ("f_not_false", "f", visual_data_frame$image, ignore.case=TRUE)
...
# ******************** II. EXTRACT RELEVANT OLD FORMAT AUDITORY DATA (ID, MODALITY, DOMAIN, TYPE, RT_SLOPE, MEAN_RT) BY CONDITION, THEN COMBINE 4 CONDITIONS TOGETHER**************************

# Separate trials 1-3 from trials 4-6----------------------------------------------------------------------------------------------------------------------------------------------------------

trials_1_3 <- old_auditory_data_frame[ which(old_auditory_data_frame$Run < 4), ]
trials_4_6 <- old_auditory_data_frame[ which(old_auditory_data_frame$Run > 3), ]
...
old_RTSL <- data.frame(part_id, task, domain,type,modality,mean_rt, upper_bound, lower_bound, rt_slope)
...
# Combine data for each participant
old_SSSL <- data.frame(part_id, task, domain,type,modality,mean_rt, upper_bound, lower_bound, rt_slope)
...
# Combine data for each participant
old_STSL <- data.frame(part_id, task, domain,type,modality,mean_rt, upper_bound, lower_bound, rt_slope)
...
# Combine data for each participant
old_RSSL <- data.frame(part_id, task, domain,type,modality,mean_rt, upper_bound, lower_bound, rt_slope)

# for internal checking only: find mean rt_slope
#mean_rssl_rt_slope <- mean (RSSL$rt_slope)


# ******************** IV. EXTRACT RELEVANT NEW AUDITORY DATA (ID, MODALITY, DOMAIN, TYPE, RT_SLOPE, MEAN_RT) BY CONDITION, THEN COMBINE 4 CONDITIONS TOGETHER**************************


# Separate trials 1-3 from trials 4-6----------------------------------------------------------------------------------------------------------------------------------------------------------

trials_1_3 <- new_auditory_data_frame[ which(new_auditory_data_frame$Run < 4), ]
trials_4_6 <- new_auditory_data_frame[ which(new_auditory_data_frame$Run > 3), ]


# ******************** CONDITION 1: STRUCTURED_SSL*******************
...
for (i in structured_ssl_targets) {
  ...
  id <- append(id, paste(trials_1_3[i,]$PartID))
  # If the participant responded while the target was presented
  if (!is.na(trials_1_3[i,] [,"sound_block_key_resp.rt"])){
    # Count their response time from the target stimulus
    rt_col <- append (rt_col, trials_1_3[i,][,"sound_block_key_resp.rt"])}
  # If the participant responded during the stimulus following the target
  else if (!is.na(trials_1_3[i+1,] [,"sound_block_key_resp.rt"])){
    ...
    rt_col[(match(i, structured_ssl_targets))] <- 1+trials_1_3[i+1, ][,"sound_block_key_resp.rt"]}
  # Check if you are looking at the first target, which is always the first stimulus. If so, it does not have a preceeding target
  else if (i>0){ 
    # Count their response time from the target stimulus (NA)
    rt_col <- append (rt_col, trials_1_3[i,][,"sound_block_key_resp.rt"])}    
  # If the participant responded during the stimulus preceding the target
    ...
    # Count their response time as how much sooner they responded than when the stimulus was presented
    rt_col[match(i, structured_ssl_targets)] <- 0-trials_1_3[i-1,][,"sound_block_key_resp.rt"]}
  # If the participant did not respond to the target within the correct time frame
  else if (is.na(trials_1_3[i,] [,"sound_block_key_resp.rt"])){
    # Count their response time from the target stimulus (NA)
    rt_col <- append (rt_col, trials_1_3[i,][,"sound_block_key_resp.rt"])}
}
...
# Remove targets with no response time
structured_ssl_extracted <- structured_ssl_extracted[!is.na(structured_ssl_extracted$rt_col),]
...
# Combine data for each participant
new_SSSL <- data.frame(part_id, task, domain,type,modality,mean_rt, upper_bound, lower_bound, rt_slope)
...
for (i in structured_tsl_targets) {
  ...
  id <- append(id, paste(trials_4_6[i,]$PartID))
  # If the participant responded while the target was presented
  if (!is.na(trials_4_6[i,] [,"tone_block_key_resp.rt"])){
    # Count their response time from the target stimulus
    rt_col <- append (rt_col, trials_4_6[i,][,"tone_block_key_resp.rt"])}
  # If the participant responded during the stimulus following the target
  else if (!is.na(trials_4_6[i+1,] [,"tone_block_key_resp.rt"])){
    # Count their response time as the duration that the target was presented (1000 ms) plus the response time to the following stimulus 
    rt_col[(match(i, structured_tsl_targets))] <- 1+trials_4_6[i+1, ][,"tone_block_key_resp.rt"]}
  # Check if you are looking at the first target, which is always the first stimulus. If so, it does not have a preceeding target
  else if (i>0){ 
    # Count their response time from the target stimulus (NA)
    rt_col <- append (rt_col, trials_4_6[i,][,"tone_block_key_resp.rt"])}    
  # If the participant responded during the stimulus preceding the target
    ...
    # Count their response time as how much sooner they responded than when the stimulus was presented
    rt_col[match(i, structured_tsl_targets)] <- 0-trials_4_6[i-1,][,"tone_block_key_resp.rt"]}
  # If the participant did not respond to the target within the correct time frame
  else if (is.na(trials_4_6[i,] [,"tone_block_key_resp.rt"])){
    # Count their response time from the target stimulus (NA)
    rt_col <- append (rt_col, trials_4_6[i,][,"tone_block_key_resp.rt"])}
}
...
# Combine data for each participant
new_STSL <- data.frame(part_id,task, domain,type,modality,mean_rt,upper_bound, lower_bound,rt_slope)
...
for (i in random_ssl_targets) {
  ...
  id <- append(id, paste(trials_4_6[i,]$PartID))
  # If the participant responded while the target was presented
  if (!is.na(trials_4_6[i,] [,"sound_block_key_resp.rt"])){
    # Count their response time from the target stimulus
    rt_col <- append (rt_col, trials_4_6[i,][,"sound_block_key_resp.rt"])}
  # If the participant responded during the stimulus following the target
  else if (!is.na(trials_4_6[i+1,] [,"sound_block_key_resp.rt"])){
    # Count their response time as the duration that the target was presented (1000 ms) plus the response time to the following stimulus 
    rt_col[(match(i, random_ssl_targets))] <- 1+trials_4_6[i+1, ][,"sound_block_key_resp.rt"]}
  # Check if you are looking at the first target, which is always the first stimulus. If so, it does not have a preceeding target
  else if (i>0){ 
    # Count their response time from the target stimulus (NA)
    rt_col <- append (rt_col, trials_4_6[i,][,"sound_block_key_resp.rt"])}    
  # If the participant responded during the stimulus preceding the target
    ...
    # Count their response time as how much sooner they responded than when the stimulus was presented
    rt_col[match(i, random_ssl_targets)] <- 0-trials_4_6[i-1,][,"sound_block_key_resp.rt"]}
  # If the participant did not respond to the target within the correct time frame
  else if (is.na(trials_4_6[i,] [,"sound_block_key_resp.rt"])){
    # Count their response time from the target stimulus (NA)
    rt_col <- append (rt_col, trials_4_6[i,][,"sound_block_key_resp.rt"])}
}
...
#rssl_test<-count(random_ssl_extracted, "id")
#rssl_test$task<- "random ssl"
