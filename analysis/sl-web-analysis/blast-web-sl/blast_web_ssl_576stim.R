...
# TO DO: This needs to be able to change with new participants
# TO DO: THis also needs to work for kids
ssl_files <- ssl_files[25:27]
...
# For ssl, the first and last stimuli in an exposure phase are never targets. Categorize if they are correct rejection or false alarm
# TO DO: Adjust the number of non-targets accordingly (ie. if they had a target appear at the beginning/end)
...
final_indiv_rts <- cbind(all_ids, mean_rt, rt_slope, hit_rate, hits, false_alarms, resp_acc, keep)
# Tidy up column names
colnames(final_indiv_rts)[colnames(final_indiv_rts)=="all_ids"] <- "part_id"

# Write individual RT results and save them to NAS
write.csv(final_indiv_rts, paste0(output_path, "online_ssl_576stim_indiv_rts.csv"))
...
write.csv(ssl_acc_table, paste0(output_path, "online_ssl_576stim_accuracies.csv"))
