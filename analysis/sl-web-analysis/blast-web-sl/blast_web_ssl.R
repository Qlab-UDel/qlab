...
#setwd("/Volumes/data-1/projects/blast/data/online_sl/blast_child")
...
ssl_entropy_key <- read.csv("/Volumes/data/projects/blast/data/online_sl/entropy_keys/ssl_entropy_key.csv")
ssl_entropy_key <- (ssl_entropy_key[c("target_type","target_order","target_occurance_order","syllable_target")])
...
# Entropy
ssl_entropy_wide<- cast(ssl_accuracy, subj~corr+triplet_type, value = "syllable_triplet", fun.aggregate = length)
...
write.csv(ssl_entropy_by_triplet[,5:6], paste0(output_path, "online_ssl_entropy_adults.csv"))
...
write.csv(ssl_acc_table, paste0(output_path, "online_ssl_accuracies.csv"))
