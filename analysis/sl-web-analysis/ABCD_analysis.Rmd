...
```{r,echo=FALSE}
#Initialize variables. Please update the total number of participants accordingly before running the script.
# ABCD_1708 missing vsl and ABCD_1727 missing tsl. There are 41 participants in total as of 10/28/2018.
...
total_test_trial <- 32 #number of 2AFC questions
```
...
TYP <- list("ABCD_1702", "ABCD_1703", "ABCD_1705", "ABCD_1708", "ABCD_1709", "ABCD_1710", "ABCD_1716", "ABCD_1720", "ABCD_1721", "ABCD_1724", "ABCD_1725", "ABCD_1727", "ABCD_1728", "ABCD_1729", "ABCD_1730", "ABCD_1732", "ABCD_1734", "ABCD_1736", "ABCD_1739", "ABCD_1742", "ABCD_1747", "ABCD_1749", "ABCD_1754", "ABCD_1783", "ABCD_1788" )

TYP_vsl <- list("ABCD_1702", "ABCD_1703", "ABCD_1705", "ABCD_1709", "ABCD_1710", "ABCD_1716", "ABCD_1720", "ABCD_1721", "ABCD_1724", "ABCD_1725", "ABCD_1727", "ABCD_1728", "ABCD_1729", "ABCD_1730", "ABCD_1732", "ABCD_1734", "ABCD_1736", "ABCD_1739", "ABCD_1742", "ABCD_1747", "ABCD_1749", "ABCD_1754", "ABCD_1783", "ABCD_1788" )

TYP_tsl <- list("ABCD_1702", "ABCD_1703", "ABCD_1705", "ABCD_1708", "ABCD_1709", "ABCD_1710", "ABCD_1716", "ABCD_1720", "ABCD_1721", "ABCD_1724", "ABCD_1725", "ABCD_1728", "ABCD_1729", "ABCD_1730", "ABCD_1732", "ABCD_1734", "ABCD_1736", "ABCD_1739", "ABCD_1742", "ABCD_1747", "ABCD_1749", "ABCD_1754", "ABCD_1783", "ABCD_1788" )
group <- NULL
...
print(vrtdd)
print(vrttyp)
vsl_rt_result <- list("vsl_rt_trial"=mean_table,"vsl_rt_subj"=subj_table)
return(vsl_rt_result)
}
```
...
```{r,echo=FALSE,warning=FALSE}
vsl_rt_result <- compute_visual()
vsl_rt_subj <- vsl_rt_result$vsl_rt_subj
...
tsl_rt_result <- list("tsl_rt_trial"=mean_table,"tsl_rt_subj"=subj_table)
return(tsl_rt_result)
}
```

#t.test for RT Slope
```{r,echo=FALSE,warning=FALSE}
tsl_rt_result <- compute_aud()
tsl_rt_subj <- tsl_rt_result$tsl_rt_subj
...
vsl_slope_typ = matrix(, nrow = 40, ncol = 2)
for(id in TYP){
  vsl_slope = matrix(, nrow = 1, ncol = 2)
  vsl_slope[,1] = id
  vsl_slope[,2] = summary(lm(rt_col~reindex,data=fam_trial_vsl[fam_trial_vsl$id==id,]))$coefficients[2]
  vsl_slope_typ = rbind(vsl_slope_typ,vsl_slope)
}
...
##In tsl
```{r,echo=FALSE}
t.test(DD_acc_tsl,TYP_acc_tsl, var.equal=TRUE)
```

##In vsl
```{r,echo=FALSE}
t.test(DD_acc_vsl,TYP_acc_vsl,var.equal=TRUE)
```

```{r,fig.width=10, fig.height=10, echo=FALSE}

Accuracy <- NULL
Accuracy <- tsl_acc_table$subj_corrt
Accuracy <- append(Accuracy,vsl_acc_table$subj_corr)
Task <- rep("TSL",total_participant)
Task <- append(Task, rep("VSL",total_participant))
ID <- paste(tsl_acc_table$list_tsl_id)
ID <- append(ID,paste(vsl_acc_table$list_vsl_id))
acc <- data.frame(ID,Accuracy,Task)
Group <- NULL
for (i in seq(from = 1, to=length(acc$ID), by=1)){Group <- append(Group,acc$ID[i] %in% DD)}
Group <- gsub(TRUE,"DD",Group)
Group <-  gsub(FALSE,"TYP",Group)
acc$Group <- Group
...
#ggplot(data=accuracy,aes(x=factor(Task), y=subj_corrt,fill=Group)) + geom_bar(position = position_dodge(),stat='summary') + 	ylab(label = 'Accuracy') + geom_beeswarm(aes(x=Task,y=subj_corrt,fill=Group), dodge.width=0.9, show.legend = FALSE) + theme_classic(base_size=20.0) + xlab(label='Task') + 	geom_hline(yintercept=0,linetype=2) + scale_fill_manual(labels = c("Dylexia","Typical"),values = c("tomato1", "#2166ac")) 
