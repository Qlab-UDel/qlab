...
acc$test_phase <- gsub ("lsl", "letter", acc$test_phase)
acc$test_phase <- gsub ("vsl", "image", acc$test_phase)

acc$task <- gsub ("ll", "same letter", acc$task)
acc$task <- gsub ("lv", "different letter", acc$task)
acc$task <- gsub ("vv", "same image", acc$task)
acc$task <- gsub ("vl", "different image", acc$task)


#ACCURACY
ggplot(data=acc,aes(x=factor(task, level=c("same letter","different letter","same image","different image")), y=accuracy, fill=factor(test_phase))) + geom_bar(stat="summary", position = position_dodge()) + theme_classic() + ylab(label="Accuracy")  +  geom_beeswarm(aes(x=factor(task, level=c("same letter","different letter","same image","different image")), y=accuracy),dodge.width=1, show.legend=FALSE) + annotate("text", label = "Same", x = 0.9, y = 1.1, color = "black")  + annotate("text", label = "Different", x = 1.97, y = 1.1, color = "black") + annotate("text", label = "Same", x = 2.9, y = 1.1, color = "black")  + annotate("text", label = "Different", x = 4, y = 1.1, color = "black") + geom_hline(yintercept = 0.5, linetype=3) + xlab(label="Task") + ylab(label="Accuracy") + scale_fill_manual(name="Test Phase",values = c("#F27645", "#51C19B"))
